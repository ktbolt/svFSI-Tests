...
class VolumeMesh(object):
    ...
    def __init__(self, file_name):
      ...
      print("[VolumeeMesh] ")
      print("[VolumeeMesh] ========== Nodes ==========")
      print("[VolumeeMesh] Number of nodes: {0:d}".format(num_points))
      for nid, point in self.nodal_coords.items():
      #for nid, point in sorted(self.nodal_coords.items()):
          print("{0:d}: {1:f} {2:f} {3:f}".format(nid, point[0], point[1], point[2]))
      ...
      print("[VolumeeMesh] ")
      print("[VolumeeMesh] ========== Cells ==========")
      num_cells = self.mesh.GetNumberOfCells()
      print("[VolumeeMesh] Number of cells: {0:d}".format(num_cells))
      ...
      for i in range(num_cells):
          ...
          for j in range(num_pts):
              ...
              ids.append(nid)
              #ids_1.append(nid)
          print("[VolumeeMesh] {0:d}: id: {1:d}  conn:{2:s}".format(i+1, elem_id, str(ids)))
...
class SurfaceMesh(object):
    def __init__(self, file_name):
      ...
      for nid, point in self.nodal_coords.items():
      #for nid, point in sorted(self.nodal_coords.items()):
          print("[SurfaceMesh] {0:d}: {1:f} {2:f} {3:f}".format(nid, point[0], point[1], point[2]))
      ...
      for i in range(num_cells):
          ...
          for j in range(num_pts):
              ...
              ids.append(nid)
              #ids_1.append(nid)
          print("[SurfaceMesh] {0:d}: id: {1:d}  conn: {2:s}".format(i+1, elem_id, str(ids)))
          #print("[SurfaceMesh] {0:d}: {1:s}".format(i+1, str(ids_1)))
